...
int bidib_switch_point(const char *point, const char *aspect) {
	if (point == NULL || aspect == NULL) {
		syslog(LOG_ERR, "Switch point: parameters must not be NULL");
		return 1;
	}
		...
		for (size_t j = 0; j < board_i->points_board->len; j++) {
			...
			if (!strcmp(point, board_mapping->id->str)) {
				if (!board_i->connected) {
					...
					syslog(LOG_ERR, "Switch point %s: board %s is not connected",
				...
				if (aspect_mapping != NULL) {
					...
					pthread_mutex_unlock(&bidib_state_boards_mutex);
					syslog(LOG_ERR, "Switch point %s: aspect %s doesn't exist",
					       point, aspect);
					return 1;
				}
			}
	...
	pthread_mutex_unlock(&bidib_state_boards_mutex);
	syslog(LOG_ERR, "Switch point %s: not found", point);
	return 1;
}

int bidib_set_signal(const char *signal, const char *aspect) {
	if (signal == NULL || aspect == NULL) {
		syslog(LOG_ERR, "Set signal: parameters must not be NULL");
		return 1;
	}
	pthread_mutex_lock(&bidib_state_boards_mutex);
	t_bidib_board *board_i;
	...
	for (size_t i = 0; i < bidib_boards->len; i++) {
		...
		for (size_t j = 0; j < board_i->signals_board->len; j++) {
			...
			if (!strcmp(signal, board_mapping->id->str)) {
				if (!board_i->connected) {
					pthread_mutex_unlock(&bidib_state_boards_mutex);
					syslog(LOG_ERR, "Set signal %s: board %s is not connected",
				...
				if (aspect_mapping != NULL) {
					...
					pthread_mutex_unlock(&bidib_state_boards_mutex);
					syslog(LOG_ERR, "Set signal %s: aspect %s doesn't exist",
					       signal, aspect);
					return 1;
				}
			}
	...
	pthread_mutex_unlock(&bidib_state_boards_mutex);
	syslog(LOG_ERR, "Set signal %s: not found", signal);
	return 1;
}

int bidib_set_peripheral(const char *peripheral, const char *aspect) {
	if (peripheral == NULL || aspect == NULL) {
		syslog(LOG_ERR, "Set peripheral: parameters must not be NULL");
		return 1;
	}
	pthread_mutex_lock(&bidib_state_boards_mutex);
	t_bidib_board *board_i;
	...
	for (size_t i = 0; i < bidib_boards->len; i++) {
		...
		for (size_t j = 0; j < board_i->peripherals->len; j++) {
			...
			if (!strcmp(peripheral, peripheral_mapping->id->str)) {
				if (!board_i->connected) {
					pthread_mutex_unlock(&bidib_state_boards_mutex);
					syslog(LOG_ERR, "Set peripheral %s: board %s is not connected",
				...
				if (aspect_mapping != NULL) {
					unsigned int action_id = bidib_get_and_incr_action_id();
					syslog(LOG_NOTICE, "Set peripheral: %s on board: %s (0x%02x 0x%02x "
							       "0x%02x 0x00) to aspect: %s (0x%02x) with action id: %d",
					...
					pthread_mutex_unlock(&bidib_state_boards_mutex);
					syslog(LOG_ERR, "Set peripheral %s: aspect %s doesn't exist",
					       peripheral, aspect);
					return 1;
				}
			}
	...
	pthread_mutex_unlock(&bidib_state_boards_mutex);
	syslog(LOG_ERR, "Set peripheral %s: not found", peripheral);
	return 1;
}

int bidib_set_train_speed(const char *train, int speed, const char *track_output) {
	if (train == NULL || track_output == NULL) {
		syslog(LOG_ERR, "Set train speed: parameters must not be NULL");
		return 1;
	}
	if (speed < -126 || speed > 126) {
		syslog(LOG_ERR, "Set train speed: illegal speed value %d, must be in range -126...126",
		       speed);
		return 1;
	}
	...
	if (tmp_train == NULL) {
		pthread_mutex_unlock(&bidib_state_boards_mutex);
		syslog(LOG_ERR, "Set train speed: train %s doesn't exist", train);
		return 1;
	} else if (board == NULL || !board->connected) {
		pthread_mutex_unlock(&bidib_state_boards_mutex);
		syslog(LOG_ERR, "Set train speed: board %s doesn't exist or is not connected", track_output);
		return 1;
		...
		unsigned int action_id = bidib_get_and_incr_action_id();
		syslog(LOG_NOTICE, "Set train: %s to speed: %d via board: %s (0x%02x "
...
int bidib_set_calibrated_train_speed(const char *train, int speed, const char *track_output) {
	if (train == NULL || track_output == NULL) {
		syslog(LOG_ERR, "Set calibrated train speed: parameters must not be NULL");
		return 1;
	}
	if (speed < -9 || speed > 9) {
...
int bidib_emergency_stop_train(const char *train, const char *track_output) {
	if (train == NULL || track_output == NULL) {
		syslog(LOG_ERR, "Emergency stop train: parameters must not be NULL");
		return 1;
	}
	t_bidib_train *tmp_train = bidib_state_get_train_ref(train);
	pthread_mutex_lock(&bidib_state_boards_mutex);
	...
	if (tmp_train == NULL) {
		pthread_mutex_unlock(&bidib_state_boards_mutex);
		syslog(LOG_ERR, "Emergency stop train: train %s doesn't exist", train);
		return 1;
	} else if (board == NULL || !board->connected) {
		pthread_mutex_unlock(&bidib_state_boards_mutex);
		syslog(LOG_ERR, "Emergency stop train: board %s doesn't exist or is not connected", track_output);
		return 1;
	} else if (!(board->unique_id.class_id & (1 << 4))) {
		pthread_mutex_unlock(&bidib_state_boards_mutex);
		syslog(LOG_ERR, "Emergency stop train: board %s is no track output", track_output);
		return 1;
	} else {
		t_bidib_cs_drive_mod params;
		params.dcc_address = tmp_train->dcc_addr;
		...
		params.function4 = 0x00;
		unsigned int action_id = bidib_get_and_incr_action_id();
		syslog(LOG_NOTICE, "Emergency stop train: %s via board: %s (0x%02x "
				       "0x%02x 0x%02x 0x00) with action id: %d",
...
int bidib_set_train_peripheral(const char *train, const char *peripheral, unsigned char state,
	...
	for (size_t i = 0; i < tmp_train->peripherals->len; i++) {
		...
		if (!strcmp(peripheral, mapping_i->id->str)) {
			...
			unsigned int action_id = bidib_get_and_incr_action_id();
			syslog(LOG_NOTICE, "Set train peripheral: %s of train: %s to "
...
int bidib_set_booster_power_state(const char *booster, bool on) {
	if (booster == NULL) {
		syslog(LOG_ERR, "Set booster power state: parameters must not be NULL");
		return 1;
	}
	pthread_mutex_lock(&bidib_state_boards_mutex);
	...
	if (board == NULL || !board->connected) {
		pthread_mutex_unlock(&bidib_state_boards_mutex);
		syslog(LOG_ERR, "Set booster: board %s doesn't exist or is not connected", booster);
		return 1;
	} else if (!(board->unique_id.class_id & (1 << 1))) {
		pthread_mutex_unlock(&bidib_state_boards_mutex);
		syslog(LOG_ERR, "Set booster: board %s is no booster", booster);
		return 1;
	} else {
		unsigned int action_id = bidib_get_and_incr_action_id();
		if (on) {
...
int bidib_set_track_output_state(const char *track_output, t_bidib_cs_state state) {
	if (track_output == NULL) {
		syslog(LOG_ERR, "Set track output state: parameters must not be NULL");
		return 1;
	}
	pthread_mutex_lock(&bidib_state_boards_mutex);
	t_bidib_board *board = bidib_state_get_board_ref(track_output);
	if (board == NULL || !board->connected) {
		pthread_mutex_unlock(&bidib_state_boards_mutex);
		syslog(LOG_ERR, "Set track output state: board %s doesn't exist or is not connected",
		       track_output);
		return 1;
	} else if (!(board->unique_id.class_id & (1 << 4))) {
		pthread_mutex_unlock(&bidib_state_boards_mutex);
		syslog(LOG_ERR, "Set track output state: board %s is no track output", track_output);
		return 1;
	} else {
		unsigned int action_id = bidib_get_and_incr_action_id();
		syslog(LOG_NOTICE, "Set track output: %s (0x%02x 0x%02x "
...
void bidib_set_track_output_state_all(t_bidib_cs_state state) {
	pthread_mutex_lock(&bidib_state_boards_mutex);
	unsigned int action_id = bidib_get_and_incr_action_id();
	syslog(LOG_NOTICE, "Set all track outputs to state: 0x%02x with action id: %d",
	       state, action_id);
	t_bidib_board *board_i;
	for (size_t i = 0; i < bidib_boards->len; i++) {
		board_i = &g_array_index(bidib_boards, t_bidib_board, i);
