...
void bidib_send_accessory_set(t_bidib_node_address node_addr, uint8_t anum,
                              uint8_t aspect, unsigned int action_id) {
	if (anum > 127) {
		syslog_libbidib(LOG_ERR, "MSG_ACCESSORY_SET called with invalid parameter anum = %02x", 
		                anum);
		return;
	} else if (aspect > 127) {
		syslog_libbidib(LOG_ERR, "MSG_ACCESSORY_SET called with invalid parameter aspect = %02x", 
		                aspect);
		return;
	}
...
void bidib_send_accessory_get(t_bidib_node_address node_addr, uint8_t anum,
                              unsigned int action_id) {
	if (anum > 127) {
		syslog_libbidib(LOG_ERR, "MSG_ACCESSORY_GET called with invalid parameter anum = %02x", 
		                anum);
		return;
	}
	uint8_t addr_stack[] = {node_addr.top, node_addr.sub, node_addr.subsub, 0x00};
...
void bidib_send_accessory_para_set_opmode(t_bidib_node_address node_addr, uint8_t anum,
                                          uint8_t anum_op, unsigned int action_id) {
	if (anum > 127) {
		syslog_libbidib(LOG_ERR, "MSG_ACCESSORY_PARA_SET (BIDIB_ACCESSORY_PARA_OPMODE) "
...
void bidib_send_accessory_para_set_startup(t_bidib_node_address node_addr, uint8_t anum,
                                           uint8_t startup_behaviour, unsigned int action_id) {
	if (anum > 127) {
		syslog_libbidib(LOG_ERR, "MSG_ACCESSORY_PARA_SET (BIDIB_ACCESSORY_PARA_STARTUP) "
		                "called with invalid parameter anum = %02x", anum);
		return;
...
void bidib_send_accessory_para_set_macromap(t_bidib_node_address node_addr, uint8_t anum,
                                            uint8_t data_size, const uint8_t *const data,
                                            unsigned int action_id) {
	if (anum > 127) {
		syslog_libbidib(LOG_ERR, "MSG_ACCESSORY_PARA_SET (BIDIB_ACCESSORY_PARA_MACROMAP) "
		                "called with invalid parameter anum = %02x", anum);
		return;
	...
	bidib_buffer_message_with_data(addr_stack, MSG_ACCESSORY_PARA_SET, 
	                               data_array_size, data_array, action_id);
}

void bidib_send_accessory_para_set_switch_time(t_bidib_node_address node_addr, uint8_t anum,
                                               uint8_t time, unsigned int action_id) {
	if (anum > 127) {
		syslog_libbidib(LOG_ERR, "MSG_ACCESSORY_PARA_SET (BIDIB_ACCESSORY_SWITCH_TIME) "
		                "called with invalid parameter anum = %02x", anum);
		return;
	}
	uint8_t addr_stack[] = {node_addr.top, node_addr.sub, node_addr.subsub, 0x00};
...
void bidib_send_accessory_para_get(t_bidib_node_address node_addr, uint8_t anum, uint8_t para_num,
                                   unsigned int action_id) {
	if (anum > 127) {
		...
		syslog_libbidib(LOG_ERR, 
		                "MSG_ACCESSORY_PARA_GET called with invalid parameter para_num = %02x", 
		                para_num);
		return;
	}
	uint8_t addr_stack[] = {node_addr.top, node_addr.sub, node_addr.subsub, 0x00};
