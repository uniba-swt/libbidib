...
void bidib_send_lc_output(t_bidib_node_address node_addr, uint8_t port0,
                          uint8_t port1, uint8_t portstat, unsigned int action_id) {
...
void bidib_send_lc_port_query(t_bidib_node_address node_addr, uint8_t port0,
...
void bidib_send_lc_port_query_all(t_bidib_node_address node_addr,
...
void bidib_send_lc_configx_set(t_bidib_node_address node_addr, uint8_t port0, uint8_t port1, 
                               uint8_t pairs_num, uint8_t *pairs, unsigned int action_id) {
	...
	bidib_buffer_message_with_data(addr_stack, MSG_LC_CONFIGX_SET, data_length, data, action_id);
}

void bidib_send_lc_configx_get(t_bidib_node_address node_addr, uint8_t port0,
                               uint8_t port1, unsigned int action_id) {
	uint8_t addr_stack[] = {node_addr.top, node_addr.sub, node_addr.subsub, 0x00};
	uint8_t data[] = {port0, port1};
...
void bidib_send_lc_configx_get_all(t_bidib_node_address node_addr, uint8_t port0, uint8_t port1,
...
void bidib_send_lc_macro_handle(t_bidib_node_address node_addr, uint8_t macro_index,
                                uint8_t opcode, unsigned int action_id) {
	if (opcode > 1 && opcode < 252) {
		syslog_libbidib(LOG_ERR, "MSG_LC_MACRO_HANDLE called with invalid parameter opcode = %02x", 
		                opcode);
		return;
	}
	uint8_t addr_stack[] = {node_addr.top, node_addr.sub, node_addr.subsub, 0x00};
...
void bidib_send_lc_macro_set(t_bidib_node_address node_addr,
...
void bidib_send_lc_macro_get(t_bidib_node_address node_addr, uint8_t macro_index,
...
void bidib_send_lc_macro_para_set(t_bidib_node_address node_addr,
                                  t_bidib_macro_params macro_params, unsigned int action_id) {
	uint8_t addr_stack[] = {node_addr.top, node_addr.sub, node_addr.subsub, 0x00};
	uint8_t data[] = {macro_params.data0, macro_params.data1, macro_params.data2,
	                  macro_params.data3, macro_params.data4, macro_params.data5};
...
void bidib_send_lc_macro_para_get(t_bidib_node_address node_addr, uint8_t macro_index,
